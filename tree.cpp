...
Node::Node(Data NewData) { _data = NewData; }
...
void Node::SetDataForDrawing(Node& root) {
  ...
  if (_data.pos < root._data.pos) {
    _data.posX =
        root.start_pos.x() - ((root._data.pos - _data.pos) * root.DrawingSize);
  } else if (_data.pos > root._data.pos) {
    _data.posX =
        root.start_pos.x() + ((_data.pos - root._data.pos) * root.DrawingSize);
  } else {
