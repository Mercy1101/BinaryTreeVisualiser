...
VisualTree::VisualTree(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::VisualTree) {
  ui->setupUi(this);
}
...
void VisualTree::paintEvent(QPaintEvent *event) {
  if (DrawTree == true) {
    ...
    std::vector<Tree::Data> nodes_data =
        _node->SetPositionOfNodesForVisualization();
    ...
    for (const auto &a : nodes_lines) {
      paint_node.drawLine(line_pos.x(), line_pos.y(), a.x() + size,
                          a.y() + size);
    ...
    for (const auto &a : nodes_data) {
      ...
      if (a.new_node == false) {
        paint_node.drawEllipse(a.posX, a.posY, _node->DrawingSize,
                               _node->DrawingSize);
      } else {
      ...
      paint_node.drawText(QRectF(QPointF(node_pos), QSizeF(QSize(34, 31))),
                          QString::number(a.val), QTextOption(Qt::AlignCenter));
    }
  }
}
